...
| ~~`codec`~~ | — | **DEIXOU de ser pacote (155).** base64 e hex são MÉTODOS de `bytes` e de `str`: `d.hex()`, `token.from_base64()`. A moldura era maior do que o quadro — 116 linhas com um `pack.json`, uma versão e um metapacote à volta —, e o `json` já estava no runtime pela mesma razão. A **155.2** escreve a linha que impede o `csv` de vir atrás |
