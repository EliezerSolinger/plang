...
reasons, neither of them being "in case pforge is not good enough".

  * **bootstrapping on a clean machine.** Whoever clones this repository without
    `plangc` or `pforge` built can run `cc bootstrap/selfhost/*.c` and `ninja`, and
    come out with the whole compiler. The `build.ninja` committed at the root is
    the first build, and after it `pforge` takes over.
...
def emit(g: G.Graph) -> str:
    out = "# generated by `pforge ninja` — do NOT edit by hand.\n"
    out += "#\n"
    out += "# This file is the BOOTSTRAP: on a machine with no `pforge` built,\n"
    out += "#   cc -O2 -o plangc bootstrap/selfhost/*.c && ninja\n"
    out += "# builds everything. After that `pforge` is in charge, and it reads\n"
