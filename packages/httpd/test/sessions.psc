...
import <httpd/sessao.psc> as ss
import sys


const CHAVE = "uma-chave-de-teste-que-nao-e-um-segredo"
...
async def handle(req: httpd.Request) -> httpd.Response:
    ...
    if req.path == "/limitado":
        if not ss.permite(ip + "|limitado", 3, 60):
            return httpd.status_code(429)
    ...
    if req.path == "/cookies":
        ...
        chaves: List<str> = []
        for k in cs:
            chaves.append(k + "=" + cs[k])
        chaves.sort()
        return httpd.text("|".join(chaves))
