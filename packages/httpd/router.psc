...
def casa(r: Route, partes: List<str>, params: Dict<str, str>) -> bool:
    ...
    while i < len(r.segs):
        ...
        if i >= len(partes):
            return False
        if s.startswith(":"):
            params[s[1:]] = httpd.pct_decode_seg(partes[i])
        elif s != httpd.pct_decode_seg(partes[i]):
            return False
        i += 1
    return i == len(partes)


async def nao_encontrado(req: httpd.Request) -> httpd.Response:
...
struct Router:
    ...
    def add(self, method: str, padrao: str, fn: def(httpd.Request) -> Task<httpd.Response>):
        self.routes.append(Route(method.upper(), split_path(padrao), fn))

    def get(self, padrao: str, fn: def(httpd.Request) -> Task<httpd.Response>):
        self.add("GET", padrao, fn)

    def post(self, padrao: str, fn: def(httpd.Request) -> Task<httpd.Response>):
        self.add("POST", padrao, fn)

    def put(self, padrao: str, fn: def(httpd.Request) -> Task<httpd.Response>):
        self.add("PUT", padrao, fn)

    def delete(self, padrao: str, fn: def(httpd.Request) -> Task<httpd.Response>):
        self.add("DELETE", padrao, fn)

    def patch(self, padrao: str, fn: def(httpd.Request) -> Task<httpd.Response>):
        self.add("PATCH", padrao, fn)

    def allowed(self, partes: List<str>) -> List<str>:
        ...
        for r in self.routes:
            p: Dict<str, str> = {}
            if casa(r, partes, p) and r.method not in out:
                out.append(r.method)
...
def router() -> Router:
    return Router([], nao_encontrado)


# O DESPACHO É UMA FUNÇÃO LIVRE, e não um método, por uma razão da linguagem: um
# método LIGADO não é um valor. `mapa.handle` não se escreve — o campo não existe
...
async def dispatch(mapa: Router, req: httpd.Request) -> httpd.Response:
    partes = split_path(req.path)
    # o método com que se PROCURA: um HEAD procura o GET, porque é o mesmo
    # handler e a diferença é só o corpo (D34)
    procurado = "GET" if req.method == "HEAD" else req.method
    melhor: Route? = None
    melhor_pontos = -1
    melhor_params: Dict<str, str> = {}
    for r in mapa.routes:
        if r.method != procurado:
            continue
        p: Dict<str, str> = {}
        if not casa(r, partes, p):
            continue
        pts = score(r.segs)
        if pts > melhor_pontos:
            melhor_pontos = pts
            melhor = r
            melhor_params = p
    if melhor != None:
        req.params = melhor_params
        return await melhor.handler(req)

    # nenhuma rota com este método. Existe com OUTRO?
    perm = mapa.allowed(partes)
    if len(perm) > 0:
        if req.method == "OPTIONS":
            r2 = httpd.status_code(204)
            r2.headers.append(httpd.Header("allow", ", ".join(perm)))
            return r2
        ...
        r3.headers.append(httpd.Header("allow", ", ".join(perm)))
        return r3
    return await mapa.not_found(req)
