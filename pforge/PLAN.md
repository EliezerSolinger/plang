# pforge / pstudio — plano de execução

**Este arquivo é o ponto de retomada** (o mesmo contrato do `pscript/PLAN.md`):
quem retomar lê ESTE arquivo primeiro, depois `pforge/DECISOES.md` (uma linha
...
3. **Enquanto `pforge verify` não der 8/8, o Makefile é a verdade.** As duas
...
> (main.p, util.p, plang.ph, api.p novo), F2 (runtime `os` + `pforge/src/*` novo)
> e F3 (`build.psc`, `pforge` — arquivos novos) são não-colidentes e é por onde eu
> vou.
...
linguagem, e não da baixa. Além de fazer o `pforge doc` mostrar pscript em vez de
...
- [x] **F1 FECHADA** (2026-08-22, com as decisões da manhã seguinte): 1.5(d),
      ...
      **A 1.5(a)** vale com `--out-dir` e não com `-o`. A medida:
      ...
      A docstring rendeu na hora: `pforge doc` (abaixo) lê a resposta 5 e mostra
      ...
      **`import <pkg/mod.ph>`**: o compilador recebe raízes de busca
      ...
      o que é versão, que é a fronteira que `pforge/ARQUITETURA.md` desenhou. As
      ...
      **1.5(d)** (a parte anterior): `import "x.ph"` dentro de um
      ...
      Seed regenerado. A bateria está em `pforge/BATERIAS.md`, à espera de o
...
- [x] **F2B — o MOTOR do pforge** (2026-08-22): `pforge/src/lib_graph.psc`
      ...
      Portão: `tests/pforge.sh` + `pforge/src/engine_test.psc`, **31 checagens**.
...
- [x] **F3 — o DESCRITOR, a biblioteca de alvos, a CLI e o ninja** (2026-08-22):
      ...
      descritor deste repositório), `pforge.psc` (a CLI),
      ...
      `pforge` construído por ele mesmo, o `verdict`, a suíte do motor, o
      ...
      **Medido**: build limpo do padrão em 68 s com `-j 6`; a suíte inteira em
      ...
      exportado com 639 regras, determinista. Portão: `tests/pforge.sh` (44
      checagens do motor + o ensaio do descritor + a exportação).
      FALTA (e está dito na secção F3): o `exec` do `pforge run` (decisão de
...
- [x] **O motor cresceu três vezes construindo de verdade** (2026-08-22), e as
      ...
      2. **o `restat` não atravessava corridas.** Guardar a data ANTIGA (para
         ...
         corrida seguinte — a aresta rodava para sempre, e o `pforge verify`
      ...
      **Medido**: `pforge build` do zero 71 s, sem mudança 7,3 s; tocar um fonte
      cujo C sai igual = 4 arestas e 7,7 s. `pforge verify` (o `verify-all`
...
- [x] **TODAS AS FASES FECHADAS, F9 inclusive** (2026-08-23, fim da sessão). A
      ...
      executor, o `pforge build --repro`, o `pforge doc --html`, os pisos dos
      placares no descritor, o `pforge lock`, e as três recusas do `publish`. É a
...
- O editor é 100 % pscript (`pstudio/lib_*.psc` + `app.psc`); em P ficou só
...
- Medições que dimensionam tudo: ver a tabela final de `pforge/DECISOES.md`.
...
F3 (descritor + CLI + troca) ────► F4 (pforge resolve por cima do build)
...
- [x] **F5 e F7 FECHADAS, e a F6 só falta a tela** (2026-08-23). A reflexão
      ...
      com VALORES e o `pforge dev`. O que resta da F6 precisa de janela: o motor
      ...
      `pforge check` (a promessa de P através dos pacotes), a faixa de toolchain
...
- [x] **F4 FECHADA** (2026-08-23): o repositório inteiro — `publish`, `keygen`,
      ...
      **72 verificações** (as três recusas do `publish` e o `pforge lock`
      entraram na F9).
- [~] **F4 começou pelo que não depende de decisão sua** (2026-08-22): o
      MANIFESTO e o WORKSPACE. `pforge/src/lib_manifest.psc` lê e valida as duas
...
**Pronto quando** tudo acima está verde e `pforge/DECISOES.md` aponta as flags.
...
- [x] **`import <pkg/mod.ph>`** (FEITA)
  ...
  - resolução: procura em cada raiz de `--pkg-path` (repetível; o pforge passa
...
- [~] baterias registradas: em `pforge/BATERIAS.md`, e não em
...
`lib_events.psc` (o contrato com as frentes). Moram em `pforge/src/` até
virarem pacote.
...
- [x] **suíte do motor** (`tests/pforge/`, harness próprio no padrão da casa):
...
**Objetivo.** O `build.psc` deste repositório, a CLI `pforge`, o `--emit-ninja`,
e a substituição do Makefile/psbuild.sh — que só acontece quando `pforge verify`
der 8/8.

### Parte A — a biblioteca de alvos (alto nível; o motor não aprende nada)

- [x] funções pscript que DEVOLVEM ARESTAS (`pforge/src/lib_targets.psc`, 470
...
- [x] a suíte do pscript: **uma aresta por caso**, cada um no diretório dele
      ...
      refaz. Quem julga é o `verdict` (`pforge/src/verdict.psc`), porque o status
...
### Parte C — a CLI `pforge` (frente da biblioteca)
...
- [x] `pforge build [alvo]` — fases descrever→planejar→decidir→executar;
      imprime linha por aresta terminada (saída inteira em falha); `--explain`;
      `-k N`; `-j N` (padrão núcleos); exit 0/1
- [x] `pforge run prog [--] args` — constrói e roda, com o status DELE e 101 para
      ...
      `pforge run` dentro de um pipe, ou a rodar um programa que lê teclado.
- [x] `pforge test` — `-k` alto por padrão (quem roda teste quer o placar
...
- [x] `pforge verify` — o `verify-all` inteiro, como GRAFO: o que não depende um
      do outro roda junto, e o que não mudou não roda
- [x] `pforge clean` — apaga o que o build produziu, MANTÉM `build/pkg`
- [x] `pforge explain <saída>` e `pforge graph` (JSON); `why`/`tree`/`lock` são
      de F4 e ficam com ela
- [x] **`pforge doc <arquivo|pacote> [símbolo]`** — a documentação no TERMINAL,
      ...
      `pforge/src/lib_api.psc` a lê de volta. É o que o `go doc` acertou —
...
- [x] **`pforge ninja`**: desce a aresta gorda para texto ninja — aspeamento
...
> linhas: ele nasce o compilador do C comitado, constrói o `pforge` com ele, e
...
- [x] `tests/psbuild.sh` — **fica, e a razão é o bootstrap** (conferido
      ...
      mão. Mas é ELE que constrói o `pforge` (`$(PPACK): $(SEED) ... bash
      tests/psbuild.sh pforge/src/main.psc`), e um `pforge` que precisasse de
      `pforge` para nascer não nasce. Ou seja: não é uma cópia do sistema de
      build, é o degrau abaixo dele — o mesmo papel que o `cc bootstrap/selfhost/
      *.c` faz para o compilador.
      A alternativa seria comitar o C gerado do `pforge` (a ideia do "seed do
      pforge" na `ARQUITETURA.md`), e ela é pior: o `bootstrap/` do compilador
      existe porque não HÁ outra forma de compilar P; para o `pforge` há uma, e
      são 69 linhas contra cem mil de C gerado num diretório versionado.
      O que os outros oito arreios ganhariam chamando `pforge` direto é
      incremental — e eles já correm DENTRO do `pforge verify`, como arestas.
- [x] `Makefile` vira casca: `make` → build do seed + `pforge build`;
      `make verify` → `pforge verify`; alvos antigos apontam e avisam
...
| f3.3 | `pforge build` sem argumento | constrói o alvo padrão do workspace |
| f3.4 | onde mora a lib de alvos | `pforge/src/lib_targets.psc` até virar pacote |

**Pronto quando** (o critério LITERAL do DESIGN): **`pforge verify` passa 8/8
...
**Objetivo.** O `pforge` resolve: manifesto, lock, workspace, os dois primeiros
...
- [x] **tipos verificáveis** (2026-08-23): `pforge check`, e ele confere as duas
...
- [x] **`packages/pui`**: extrair `lib_pui.psc` (zero import — medido) +
      ...
      saem de `tests/pstudio/` para dentro do pacote; `pforge test` roda testes
...
- [x] **`index.json`**: por pacote/versão — metadado do manifesto + SHA-256 do
      ...
      `pforge index <dir>` (o comando que mantém um repositório-diretório)
- [x] **transportes**: `file://` e `http://` (o cliente HTTP conformante já
      existe); `pforge update` baixa o índice do(s) repos configurados e guarda
      em `build/pkg/index/` (offline depois)
- [x] **`pforge search <termo>`**: por nome, descrição E SÍMBOLO (a lista de API
      no índice — o diferencial decidido)
- [x] **`pforge add nome@versão`** (mexe manifesto+lock, baixa p/ hash, NÃO
      constrói), **`pforge up [nome]`** (versão exata nova, mostra diff de API
      entre as duas versões — as listas estão no índice), **`pforge why`**
- [x] **`pforge publish`**: monta o tarball (com `test/`, sem `build/`),
...
- [x] **Ed25519 em P** (~500 linhas, aritmética pura; vetores RFC 8032 no
      ...
      binário; `pforge key` gera/instala chaves de terceiros
...
| f4.4 | `pforge index` assina como | flag `--sign <chave>` |

**Pronto quando**: um projeto FORA deste repo faz `pforge add pui@0.1.0` de um
...
- [x] **tabela de campos no `PsDesc`** (FEITA, 2026-08-23):
  ...
  - custo MEDIDO: +1,4 % no `pforge.c` e +2,6 % no `app.c` do editor — dentro do
...
- [~] baterias em `pforge/BATERIAS.md` (o `pscript/DESIGN.md` carrega trabalho
...
| f5.3 | flag do diagnóstico JSON | `--diag-json` (para não colidir com `--json` da CLI pforge) |
...
- [x] **pforge em processo** (FEITO, 2026-08-23): `packages/pforge` é uma
      ...
      **Uma coisa saiu diferente do plano, e a favor**: o grafo vem
      SERIALIZADO (`pforge graph`), e não montado no editor. A razão é que o
...
- [x] **painel de build** (FEITO, 2026-08-23) — **e não é um formulário.**
      ...
      `pforge`). A paleta aprendeu a PERGUNTAR (`PAL_TEXTO`), que é a quarta coisa
      que se faz com a linha de entrada que já procurava arquivo, comando e
      linha.
      A cirurgia que escreve no `pack.json` sem o estragar saiu do `pforge` para o
...
- [x] **play/vassoura** (FEITO, 2026-08-23): na PALETA que o editor já tinha,
      ...
      `pforge`. O `Run` constrói e LANÇA (`os.spawn`), mata o anterior por
...
- [~] **`pforge dev`** (2026-08-23): constrói, espera que alguma coisa mude, e
...
- [x] **erros como POSIÇÃO E MARCA** (2026-08-23): o editor lê
      ...
      compilador já usa e que o `pforge` copiou de propósito —, abre o arquivo e
...
`pforge dev` reconstrói e relança ao salvar. O que falta é **o que precisa de olhos** — o painel que edita o
...
depois de `pforge run` maduro.

- [x] `tests/run-cmd-pforge.sh` mede `pforge run` (9 checagens, os MESMOS
...
promessa "um binário roda scripts" passou oficialmente ao `pforge`.
...
`ARQUITETURA.md`, `DECISOES.md`, `pforge/PACOTES.md` e `pforge/REPOSITORIO.md`,
...
- [x] **`pforge build --repro`.** Constrói duas vezes, do zero, e compara byte a
...
- [x] **`pforge doc --html <pasta>`.** O mesmo conteúdo do terminal como site
      estático, da mesma resposta 5. A caminho apareceu um defeito com dono: o
      `--api` do `pforge doc` nunca passava `--pkg-path`, então todo módulo que
...
- [x] **os PISOS dos placares no descritor.** `c-suite >= 220` e
      ...
      que lê o número e compara (`pforge/src/floor.psc`) — e as duas suítes
      entram no `pforge verify`. O arreio LÊ o piso do descritor: um lugar só
      onde se sobe um número.
- [x] **`pforge lock`.** Estava na lista de comandos da v1 e não existia. Refaz o
...
- [x] **o contador do relatório não recomeçava.** `feitas` e o placar são
      ...
      `pforge dev` fazia isso a cada mudança.
...
- [ ] **QBE embutido, forma (2)+(3)**: o C do QBE (18 220 linhas) compilado
      ...
      por buffer ao parser dele; `pforge run` frio cai de ~3,3 s para décimos;
...
| F2B | `pforge/src/*` (novo), `tests/pforge/` (novo) | tudo o mais |
| F3 | `pforge/src/*`, `build.psc` (novo), `Makefile`, `tests/*.sh`, `.gitignore` | compilador |
| F4 | `packages/*`, `selfhost` (imports `<stl/...>`) + **seed**, runtime (sha256), `pforge` | — |
| F5 | `psrt_types.ph`, `psrt_val/rt.p`, `ps_lower.p`, `util.p` (diag), `pforge` | — |
| F6 | `pstudio/*`, runtime (`os.watch`), `pforge dev` | compilador |
...
5. **Dois donos durante F3**: até `pforge verify` 8/8, o Makefile é a verdade;
   nenhum harness passa a chamar pforge antes da Parte D.
