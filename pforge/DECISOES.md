# Índice de decisões — pforge, packages e a linguagem

Uma linha por decisão, tudo de 2026-08-21/22. O detalhe e o *porquê* estão nos
documentos apontados; isto é o mapa.

> **Os nomes mudaram em 2026-08-24 (F1 do `pstudio/PLAN.md`).** Havia duas
> pastas — `pbuild/` para o motor de build e `ppack/` para o gerenciador de
> pacotes — e UM binário que fazia as duas coisas, chamado `ppack`. Suas
> palavras: *"se um executável apenas faz os dois e esse for o melhor design, o
> nome deveria englobar os dois conceitos em uma única palavra"*. A palavra é
> **`pforge`**: uma forja faz coisas a partir de matéria-prima, e "forge" carrega
> também o sentido de onde o código mora.
>
> Hoje: uma pasta `pforge/` com os documentos e `src/`, o pacote
> `packages/pforge` (o motor E o gerenciador, como biblioteca), e o binário
> `build/bin/pforge`. **As citações abaixo ficam verbatim** — onde você disse
> "ppack" ou "pbuild", está escrito "ppack" ou "pbuild", porque um registo
> reescrito deixa de ser um registo.

`D` = `pforge/DESIGN.md` · `A` = `pforge/ARQUITETURA.md` · `L` = `pforge/LINKER.md`
· `P` = `pforge/PACOTES.md` · `K` = `packages/README.md`

**O plano de execução — fases, ordem, critérios de pronto — é `pforge/PLAN.md`**,
...
| `pforge` | **decide** — é uma **biblioteca**, e ela **não imprime**: relata eventos |
| `pforge` | **resolve** — e é o comando de cima (`pforge build/run/test/...`) |
...
| — | falha: **para na primeira**, `-k N` continua; `pforge test` roda com `-k` |
| — | teste: **suítes nomeadas**, sempre rodam (veredicto não cacheado), **pisos no descritor** |
| — | `verify-all` vira **alvo do descritor** (`pforge verify`) — critério de pronto da v1 |
...
| — | `pforge dev` reconstrói **e reinicia**; `build --repro` constrói duas vezes e compara |
...
| — | erros do pforge no **estilo do compilador** (`arquivo:linha:col:`) |
...
| — | o **`run` sai do compilador** (passo tardio, depois de o `pforge run` existir) |
...
| — | `pforge run`: constrói e **o programa é o processo** |
...
`packages/` é o que **nós escrevemos**; `build/pkg/` é o que o pforge **baixou**.
...
com o `pforge` ser uma lista de diretórios em vez de um protocolo.
...
| **o build do padrão pelo `pforge`** | 71 s do zero · 7,3 s sem mudança |
| **`pforge verify`** (o verify-all inteiro) | **5m48 do zero · 7,7 s sem mudança** |
