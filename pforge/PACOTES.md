# pforge — os PACOTES: documento de decisões

(o motor de build tem o seu, em `pforge/DESIGN.md`)
...
`pforge/gerenciador-de-pacotes-ideal.md` (feita por outro agente — os erros
históricos e o desenho ideal) e o que o repositório **já é**, medido enquanto se
investigava o `pforge`. As perguntas ficam no fim, numeradas, como em
`pscript/DESIGN.md` e `pforge/DESIGN.md`.
...
| **`pforge`** (build) | **decide**: o que está velho, a ordem, o paralelismo, e roda | ir à rede; saber o que é uma "versão" |
| **`pforge`** (pacotes) | **resolve**: que versão de que pacote, baixa, tranca no lock | ter um motor de build próprio |
...
metade que vem a maior parte da complexidade dele. Aqui o `pforge` já vai existir
antes, com grafo, sujeira por conteúdo e paralelismo. **O `pforge` acrescenta duas
arestas ao grafo que já existe** — "baixar" e "verificar" — e nada mais. É o mesmo
princípio da forma decidida no pforge (um binário, dois modos, como o muon
...
**2. O hash de interface (1.5b do pforge) É o verificador de semver.** O ponto 9
...
- interface diferente → **não é patch**, e o `pforge` sabe disso sem heurística;
...
**já está implementado**, dentro do compilador, para um caso só. O `pforge` não
...
**2.1 O que É um pacote?** A resposta depende da 1.5(a) do pforge (`import`
...
ao `pforge`. A pesquisa pede doc-comments reconhecidos pelo compilador e um JSON
...
(pforge para sujeira, pforge para semver, o site/offline para doc). Entra no
...
hoje mora em seis lugares. Ou seja o `pforge` **não pode existir** com essa lista
fora da linguagem: a **1.5(a) do pforge é pré-requisito dele**, não melhoria.
...
| índice pesquisável offline | ✅ `json` + `dict` + o índice O(1) da 80.1b; é `pforge update` guardando um arquivo |
...
```
 fase 1  RESOLVER   pforge    lê o manifesto + o lock, fala com os repositórios,
                             baixa e VERIFICA, materializa uma árvore de pacotes
                             ---- daqui para frente, ZERO rede ----
 fase 2  PLANEJAR   pforge   pergunta ao plangc (o que você lê? o que vai emitir?
                             qual o hash de cada interface? quem você é?) e monta o grafo
 fase 3  CONSTRUIR  pforge   decide o que está velho, ordena, roda em paralelo
```
...
**`pforge` → `pforge`: duas arestas novas e um tipo de nó novo.** O `pforge` não
...
**`pforge` → `plangc`: o compilador NUNCA ouve a palavra "versão".** A saída da
...
**`plangc` → `pforge`: a mesma resposta que ele já dá ao `pforge`.** O hash de
interface serve ao `pforge` para não recompilar e ao `pforge` para dizer se um
...
> "preciso da lib `sdl2`". Quem chama o `pkg-config` é o `pforge`/`pforge`, não o
...
- **como a primeira chave chega** (o problema do chaveiro de distro): a chave
  pública do repo padrão vem embutida no `pforge` — o que é honesto e faz do
...
(`pforge up foo`). **Não há resolvedor na v1** — e com versão única no grafo (2.2),
...
## A faixa de toolchain — DECIDIDO: o pforge confere na fase 1
...
que ninguém oferece caem de graça: `pforge search draw_rect` **procura por
...
**`pforge run`: constrói o que estiver sujo e depois o programa É o processo.**
...
que permite `pforge run` dentro de um pipe. Duas exigências que isso impõe: o
...
que tinha e vira o que já foi decidido — casca fina sobre `pforge verify`.
...
**depois** de o `pforge run` existir e ser bom — é passo tardio, não inicial; (b) o
`tests/run-cmd.sh` existe para medir exatamente esses comportamentos (cache, o que
invalida, o status de saída) e passa a medir o `pforge`; (c) rodar um `.psc` passa a
exigir o `pforge` — a promessa de "um binário, sem dependência" migra para ele.
...
ser, para o `pforge`, um projeto como qualquer outro — e é assim que o ecossistema
é testado por quem o escreve, contra o caso mais difícil que existe.

**As mensagens de erro do pforge seguem o estilo do compilador**:
...
(`-t graph`, `-t query`, `-t targets`, `-explain`). O `pforge why sdl2` responde
"quem puxou isto", que é a pergunta que todo lock grande acaba provocando.

**`pforge add pui@0.1.0` mexe no manifesto e no lock, e NÃO constrói.** Uma coisa
...
docstrings, então `pforge doc pui.draw_rect` no terminal é quase grátis (é o que o
