...
`pforge/DESIGN.md` e `pforge/PACOTES.md` já implicam, e isola o que **ainda não
...
pforge  DECIDE     o que está velho · em que ordem · com quantos processos · e roda
pforge   RESOLVE    que versão · de que repositório · com que hash · e materializa
```
...
| **pforge** | pscript | **sim** — 6 módulos e um coletor | ? (é a pergunta A) |
| **pforge** | pscript | sim (vive dentro do pforge) | idem |
...
e o C do runtime do pscript são 8 297. Enfiar o pforge dentro do `plangc` levaria
...
pforge — a forma do `build.zig`, e a que já decidimos. A pergunta óbvia
...
O ovo e a galinha se resolve com **uma receita fixa dentro do pforge** para
...
```
1. RESOLVER   pforge    manifesto + lock → repos (arquivo/http/ftp) → SHA-256 → índice assinado
                       → árvore de pacotes materializada + caminho de busca
                       ─────────── daqui para frente, ZERO rede ───────────
2. DESCREVER  pforge   roda o build.psc (do armazém) → grafo em MEMÓRIA (1.8)
3. PLANEJAR   pforge   pergunta ao plangc, por raiz: o que lê · o que emite ·
                       hash de cada interface · quem ele é → nós e arestas
4. DECIDIR    pforge   os 6 testes do ninja · restat e poda · hash de comando+env
5. EXECUTAR   pforge   fila por caminho crítico (com a duração da última vez, 1.7),
                       N processos, saída de cada job capturada e despejada inteira
6. GRAVAR     pforge   log local (durações, hashes) — e o lock só muda na fase 1
```
...
do pforge (e aí a pergunta A fica carregada: o caminho JIT precisa das duas peças
no mesmo processo).
...
**A — DECIDIDA: o `pforge` orquestra.** Suas palavras: *"já que o ppack é o comando
que gerencia o pacote, faz ele orquestrar também"*. Então:

- **`pforge` é o comando de cima** (o papel que o `cargo` faz), e o **pforge é o
  motor dentro dele** — o que já era a forma decidida no `pforge/DESIGN.md` (um
...
- **o `pforge` tem o seu próprio seed** — o C gerado dele, comitado — que é o
...
**B — SIM, o pforge pode e deve orquestrar o cache.** Sua pergunta: *"o pbuild não
pode orquestrar cache também? porque não?"* — pode, e a razão de estar hoje no
compilador é só cronológica: o pforge não existia. Veja o que o `run` faz hoje:

1. hash de cada fonte que leu · 2. hash dos bytes do próprio compilador ·
3. chave → binário no armazém · 4. `exec`

**Os três primeiros são exatamente o que o pforge vai fazer para toda aresta**, e
o quarto é trabalho do comando de cima. Não há mecanismo novo aqui: há mecanismo
**duplicado**, dentro do compilador. Passá-lo ao pforge mata a sexta lista (o
...
ora"* — e a pesquisa do pforge diz o mesmo no ponto 3 (*isolamento por projeto por
...
**A′ — o comando é `pforge`**: `pforge build`, `pforge run`, `pforge add`,
`pforge test`. O nome da peça que orquestra é o nome que se digita.
...
pstudio   (IDE: play, vassoura, configuração)   ← nada depende dele
   |
   v
pforge     (resolve, e é o comando de cima)
   |
   v
pforge    (decide e executa)
   |
   v
plangc    (responde e compila)
```
...
pforge são os dois pscript e viram um binário, o botão de play não *invoca* um
...
- **a saída de cada aresta é capturada inteira** (1.2/1.6c) — para um painel de
  ...
  intercala com nada;
- **`pforge dev`** (reconstruir e reiniciar) é o botão de play com o programa
...
> **O pforge é uma BIBLIOTECA. O `pforge` e o `pstudio` são as suas duas frentes.**
...
| # | o pforge/pstudio pergunta | o plangc responde |
|---|---|---|
...
**`pforge publish` empacota, assina e RECUSA.** Tarball do fonte, SHA-256,
