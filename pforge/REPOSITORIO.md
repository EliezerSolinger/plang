...
> `pforge/PACOTES.md`, mede o que existe hoje para as pagar, e leva cada uma até a
...
| o **índice carrega a lista canónica da API** — `pforge search` por SÍMBOLO, offline | DESIGN, §mais quatro |
...
| quem chama o `pkg-config` é o **pforge**, nunca o pacote | DESIGN 2.7 |
| a faixa de **toolchain** é conferida na fase 1, antes de compilar | DESIGN, §toolchain |
| `pforge run` constrói e depois **o programa É o processo** (`exec`) | DESIGN, §mais quatro |
...
| o `.tar` leva a árvore inteira **menos uma lista fixa** que o `pforge` decide | esta sessão |
| índice velho → **recusa** e manda rodar `pforge update`; a rede é quando você manda | esta sessão |
...
| manifesto e workspace | ✅ `packages/pforge/lib_manifest.psc`, com erro posicionado |
| grafo de pacotes, `tree`, `why` | ✅ `packages/pforge/lib_pkg.psc` |
...
que faz `pforge search draw_rect` procurar **por símbolo** sem baixar nada, e a
...
E um pacote solto também: `pforge add exp@0.1.0 --unsafe`. As duas formas existem
...
```
pforge keygen <arquivo>         uma chave nova: privada + .pub        (local)
pforge update                   baixa os índices e guarda-os          (rede)
pforge search <termo>           procura no índice guardado            (offline)
pforge add <nome>@<versão>      escreve no manifesto e no lock        (rede: só para o hash)
pforge install [--frozen]       materializa o que o lock diz          (rede se faltar)
pforge up [<nome>]              sobe a versão e regrava o lock        (rede)
pforge publish <pkg> --to <dir> faz o .tar, assina, escreve no índice (local)
pforge check                    as invariantes dos pacotes            (local)
```
...
3. ✅ **`pforge publish` e um repositório `file://`** — `publish <pacote> --to
...
4. ✅ **`pforge update`/`search`/`add`/`install` sobre `file://`** — a fase 1
...
6. ✅ **Ed25519 e as duas assinaturas** — `packages/ed25519`, RFC 8032, com os
   ...
   padding do SHA-512 se esconde). `pforge keygen` faz a chave a partir do
...
## 9. `pforge check`: as invariantes que o build não confere
