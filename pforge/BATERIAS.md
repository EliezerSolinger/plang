...
As duas foram achadas rodando a suíte do motor do pforge com
...
resolução é o `pforge`, e a fronteira entre os dois é literalmente esta lista de
diretórios.
...
Uma consequência boa: `plangc --api` já é o alimento do `pforge doc` e do gerador
...
quer sublinhar a coluna certa sem reparsear texto; o `pforge` quer contar e
...
zero, isto é, com **sucesso**, sem uma linha de mensagem. Um `pforge` que morresse
...
Duas consequências, e a segunda é a séria:
  ...
  * `pforge doc` mostrava o C, e não o pscript — o que faz a documentação de um
...
Custo MEDIDO: +1,4 % no `pforge.c` e +2,6 % no `app.c` do editor, contra um alvo
