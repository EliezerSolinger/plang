"""The pbuild ENGINE, mechanism by mechanism (F2B).
...
async def case_restat():
    ...
    # `ppack verify` redid 296 edges for nothing. That is how it showed up.
    reset()
    g3 = G.new_graph()
...
async def case_api():
    """Answer 5 read back (`lib_api.psc`): it is what makes `ppack doc` exist
