...
async def serve_manifest(app: appm.App):
    ...
    **A dependency** is NOT written here: it is asked of `pforge`, which fetches
    ...
    if len(app.want_manifest_dep) > 0:
        ...
        r = await os.run([where_is_pforge(), "add", request], cwd=app.root_dir)
        if r.status() == 0:
...
async def selftest(arg: str) -> int:
    ...
    app.build_targets = ["build/bin/plangc_s2", "build/bin/pforge"]
    app.palette_open(appm.PAL_BUILD)
    u.set_text(app.palinput, "!pforge")
    app.palette_filter()
...
async def mode_manifest(arg: str) -> int:
    ...
    app.want_manifest_default = "build/bin/pforge"
    await serve_manifest(app)
    print(app.build_msg)
