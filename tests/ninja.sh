...
#
# It is the BOOTSTRAP: on a machine with no `pforge`, `cc -O2 -o plangc
...
"$PFORGE" ninja "$OUT/build.ninja" >/dev/null 2>&1 || { echo "   ninja: the emitter failed"; exit 1; }
