#!/usr/bin/env bash
# tests/run-cmd-pforge.sh — `pforge run`, and the cache behind it (F7).
#
...
case $PFORGE in /*) ;; *) PFORGE=$PWD/$PFORGE;; esac
OUT=tests/out/runpforge
...
lines=$("$PFORGE" run "$OUT/prog.psc" abc 2>/dev/null | grep -c '^\[' || true)
...
EOF
got2=$("$PFORGE" run "$OUT/prog.psc" 2>/dev/null | tail -1)
...
def main() -> int:
    ...
    return 0
EOF
got4=$("$PFORGE" run "$OUT/inp.p" 2>/dev/null | tail -1)
check "a program in P runs the same" "in P" "$got4"

# 8. a LOOSE script's binary is born NEXT TO IT (architecture C′): `pforge run
...
rm -rf "$OUT/elsewhere"
"$PFORGE" run --build-dir "$OUT/elsewhere" "$OUT/prog.psc" >/dev/null 2>&1
...
"$PFORGE" run "$OUT/doesnotexist.psc" >/dev/null 2>&1 && { echo "  FAIL a file that does not exist should fail"; fail=$((fail+1)); } || ok=$((ok+1))

echo "   run-pforge: $ok ok, $fail failed"
[ $fail = 0 ]
